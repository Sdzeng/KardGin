...
func CreateYamlFactory(fileName ...string) interf.YmlConfigInterf {
	...
	// 配置文件所在目录
	yamlConfig.AddConfigPath(variable.ConfigPath)
	// 需要读取的文件名,默认为：config
	if len(fileName) == 0 {
	...
	if err := yamlConfig.ReadInConfig(); err != nil {
		log.Fatal(kardError.ErrorsConfigInitFail + err.Error())
	}
...
func (y *ymlConfig) keyIsCache(keyName string) bool {
	if _, exists := cache.CreateContainersFactory().KeyIsExists(variable.ConfigKeyPrefix + keyName); exists {
		return true
	} else {
		return false
	}
...
func (y *ymlConfig) cache(keyName string, value interface{}) bool {
	return cache.CreateContainersFactory().Set(variable.ConfigKeyPrefix+keyName, value)
}

// 通过键获取缓存的值
func (y *ymlConfig) getValueFromCache(keyName string) interface{} {
	return cache.CreateContainersFactory().Get(variable.ConfigKeyPrefix + keyName)
}

// 清空已经窜换的配置项信息
func (y *ymlConfig) clearCache() {
	cache.CreateContainersFactory().FuzzyDelete(variable.ConfigKeyPrefix)
}
...
func (y *ymlConfig) Clone(fileName string) interf.YmlConfigInterf {
	...
	if err := (&ymlC).viper.ReadInConfig(); err != nil {
		variable.ZapLog.Error(kardError.ErrorsConfigInitFail, zap.Error(err))
	}
	return &ymlC
}
