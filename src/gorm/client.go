...
func GetOneMysqlClient() (*gorm.DB, error) {
	sqlType := "Mysql"
	readDbIsOpen := variable.GormYml.GetInt("Gormv2." + sqlType + ".IsOpenReadDb")
...
func GetSqlDriver(sqlType string, readDbIsOpen int, dbConf ...ConfigParams) (*gorm.DB, error) {
	...
	if val, err := getDbDialector(sqlType, "Write", dbConf...); err != nil {
		variable.ZapLog.Error(kardError.ErrorsDialectorDbInitFail+sqlType, zap.Error(err))
	} else {
		dbDialector = val
	}
	...
	if readDbIsOpen == 1 {
		...
		err = gormDb.Use(dbresolver.Register(resolverConf).SetConnMaxIdleTime(time.Second * 30).
			SetConnMaxLifetime(variable.GormYml.GetDuration("Gormv2."+sqlType+".Read.SetConnMaxLifetime") * time.Second).
			SetMaxIdleConns(variable.GormYml.GetInt("Gormv2." + sqlType + ".Read.SetMaxIdleConns")).
			SetMaxOpenConns(variable.GormYml.GetInt("Gormv2." + sqlType + ".Read.SetMaxOpenConns")))
		if err != nil {
			return nil, err
		}
	}
	...
	if rawDb, err := gormDb.DB(); err != nil {
		...
		rawDb.SetConnMaxLifetime(variable.GormYml.GetDuration("Gormv2."+sqlType+".Write.SetConnMaxLifetime") * time.Second)
		rawDb.SetMaxIdleConns(variable.GormYml.GetInt("Gormv2." + sqlType + ".Write.SetMaxIdleConns"))
		rawDb.SetMaxOpenConns(variable.GormYml.GetInt("Gormv2." + sqlType + ".Write.SetMaxOpenConns"))
		return gormDb, nil
	}
}
...
func getDbDialector(sqlType, readWrite string, dbConf ...ConfigParams) (gorm.Dialector, error) {
	...
	default:
		return nil, errors.New(kardError.ErrorsDbDriverNotExists + sqlType)
	}
...
func getDsn(sqlType, readWrite string, dbConf ...ConfigParams) string {
	Host := variable.GormYml.GetString("Gormv2." + sqlType + "." + readWrite + ".Host")
	DataBase := variable.GormYml.GetString("Gormv2." + sqlType + "." + readWrite + ".DataBase")
	Port := variable.GormYml.GetInt("Gormv2." + sqlType + "." + readWrite + ".Port")
	User := variable.GormYml.GetString("Gormv2." + sqlType + "." + readWrite + ".User")
	Pass := variable.GormYml.GetString("Gormv2." + sqlType + "." + readWrite + ".Pass")
	Charset := variable.GormYml.GetString("Gormv2." + sqlType + "." + readWrite + ".Charset")
